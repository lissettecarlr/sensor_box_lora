...
void OnRadioTxDone( void )
{
    ...
    DEBUG_NORMAL(" Send Frame Done \r\n");

    if( IsLoRaMacNetworkJoined == false )
    {
        LedRadioTxJoinRequestShow();
    }
    else
    {
        LedRadioTxDataShow();
...
#if 0
                DEBUG_NORMAL(" AppNonce:%06X \r\n", LoRaMacAppNonce);
                DEBUG_NORMAL(" DevNonce:%04X \r\n", LoRaMacDevNonce);
                DEBUG_NORMAL(" NetID:%06X \r\n", LoRaMacNetID);
                DEBUG_NORMAL(" DevAddr:%08X \r\n", LoRaMacDevAddr);
                DEBUG_NORMAL( " NwkSKey:" );
                for( i = 0; i < 16; i++ )
                {
                    DEBUG_NORMAL( "%02X", LoRaMacNwkSKey[i] );
                }
                DEBUG_NORMAL( "\r\n" );
                DEBUG_NORMAL( " AppSKey:" );
                for( i = 0; i < 16; i++ )
                {
                    DEBUG_NORMAL( "%02X", LoRaMacAppSKey[i] );
                }
                DEBUG_NORMAL( "\r\n" );
#endif
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {
                ...
                if( isMicOk == true )
                {
                    ...
                    else
                    {
                        ...
                        {
                            DownLinkTotal++;
                            DEBUG_NORMAL( " Need %d packs,Rx %d packs,loss %.2f%% \r\n", DownLinkCounter + 1, DownLinkTotal,
...
static bool SetNextChannel( TimerTime_t* time )
{
    ...
    { // Make sure, that the channels are activated
        ChannelsMaskRemaining[4] = 0x00FF;
    }
#elif defined( USE_BAND_470 )
...
static bool RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    ...
    if( Radio.GetStatus( ) == RF_IDLE )
    {
        ...
        {
//            DEBUG_NORMAL( " Start RF Recv[%uHz FSK st:%d]... \r\n", freq, timeout );
        }
        else
        {
            DEBUG_NORMAL( "\r\nStart RF Recv[%uHz SF%d %dK st:%d]\r\n", freq, downlinkDatarate, (bandwidth==0)?125:(bandwidth== 1)?250:500, timeout );
        }
        return true;
    }
    return false;
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            if( ( payload != NULL ) && ( LoRaMacTxPayloadLen > 0 ) )
            {
                if( ( MacCommandsBufferIndex <= LORA_MAC_COMMAND_MAX_LENGTH ) && ( MacCommandsInNextTx == true ) )
                {
                    ...
                    {
                        DEBUG_NORMAL( " %02X", MacCommandsBuffer[i] );
                    }
                    DEBUG_NORMAL( " ]\r\n" );
#endif
                }
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    if( datarate == 50 )
    {
        DEBUG_NORMAL( "\r\n [%uHz FSK %ddBm SIZE:%d] Send Frame->", channel.Frequency, txPower, LoRaMacBufferPktLen );
    }
    else
    {
			DEBUG_NORMAL( "\r\nSend->>>[%uHz SF%d %dK %ddBm SIZE:%d] Frame:[", channel.Frequency, datarate, (bandwidth==0)?125:(bandwidth== 1)?250:500, txPower, LoRaMacBufferPktLen );
    }
    for( uint16_t i = 0; i < LoRaMacBufferPktLen; i++ )
    {
        DEBUG_NORMAL( " %02X", LoRaMacBuffer[i] );
    }
    DEBUG_NORMAL( "]\r\n" );
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    // Initialize timers
    TimerInit( &MacStateCheckTimer, OnMacStateCheckTimerEvent, "mac state check" );
    TimerSetValue( &MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT );

    TimerInit( &TxDelayedTimer, OnTxDelayedTimerEvent, "tx delayed" );
    TimerInit( &RxWindowTimer1, OnRxWindow1TimerEvent, "rx window 1" );
    TimerInit( &RxWindowTimer2, OnRxWindow2TimerEvent, "rx window 2" );
    TimerInit( &AckTimeoutTimer, OnAckTimeoutTimerEvent, "ack timeout" );
