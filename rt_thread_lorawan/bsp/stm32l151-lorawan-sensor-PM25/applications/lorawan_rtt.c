...
// 定时器回调，当此时大于阈值则不进行入网
#define DEFAULT_JOIN_TIMES  0XFFFF
static uint32_t default_join_times = 0;
			  ...
			  if(event & LORAWAN_EVENT_JOIN)
                ...
                //更新传感器
							  ...
							  if(NodeMsg.CycleS >= 60 * 60) //如果发送周期大于一个小时，则每个次上传都保存一次数据
								...
								lorawan_send(SendInfo.SendBuffer,SendInfo.SendBufferSize,SendInfo.mode,11);
